    ...
    <header className="h-16 bg-slate-900 border-b border-slate-800 px-4 flex items-center justify-between select-none z-30">
      ...
      <div className="flex items-center bg-slate-950/80 border border-slate-800/80 p-1 rounded-xl gap-1">
        <button
          ...
          className="flex items-center gap-1 px-2.5 py-1 text-slate-300 hover:text-white hover:bg-slate-800 rounded-lg text-xs transition"
          title="Añadir texto en arco circular"
        >
          <Type size={14} className="text-sky-400" />
          <span>Texto en Arco</span>
        </button>

        <button
          onClick={() => onAddLayer('center-text')}
          className="flex items-center gap-1 px-2.5 py-1 text-slate-300 hover:text-white hover:bg-slate-800 rounded-lg text-xs transition"
          title="Añadir texto horizontal al centro"
        >
          <Baseline size={14} className="text-indigo-400" />
          <span>Texto Central</span>
        </button>

        <button
          onClick={() => onAddLayer('frame')}
          className="flex items-center gap-1 px-2.5 py-1 text-slate-300 hover:text-white hover:bg-slate-800 rounded-lg text-xs transition"
          title="Añadir círculo / marco de borde"
        >
          <CircleDot size={14} className="text-emerald-400" />
          <span>Borde / Círculo</span>
        </button>

        <button
          onClick={() => onAddLayer('icon')}
          className="flex items-center gap-1 px-2.5 py-1 text-slate-300 hover:text-white hover:bg-slate-800 rounded-lg text-xs transition"
          title="Añadir icono vectorial o escudo"
        >
